...
- [X] T009 [P] In `scripts/install.sh`, after identity generation and Apache config rendering, add `mkdir -p "${DATA_DIR}/drop"` (idempotent; noop if exists; 0750 mode recommended).
...
- [X] T025 [P] [US4] Implement `cli/_common-cname.sh` — a shared bash helper. Given `--cert <F>` (or fallback `MTLS_CLIENT_CERT`), runs `openssl x509 -in "$cert" -noout -subject -nameopt RFC2253` and prints the CN (sanitized to `[A-Za-z0-9._-]+`, ≤128 chars). Falls back to env. Refuses (exit `2`) on failure with a clear stderr message. Sourceable.
- [X] T026 [P] [US4] Implement `cli/mtls-drop.sh` (PUT) — args `--source`, `--name`, `--content-type`, `--etag`, `--if-none-match`, `--server`, `--cert`, `--key`, `--cacert`; sources `_common-cname.sh`, derives CN, builds `/drop/<cn>/<name>`; `curl --fail-with-body --cert ... --key ... --cacert ... -X PUT -T <local>`; emits `(201 created <name>)` / `(204 overwritten <name>)` / `(412 precondition failed)`. Default content-type via `file --mime-type`.
- [X] T027 [P] [US4] Implement `cli/mtls-fetch.sh` (GET) — `--name`, `--out`, `--range <A-B>`, `--if-none-match <etag>`, `--server`, `--cert`, `--key`, `--cacert`. `curl -X GET --cert ... --cacert ... -D /tmp/h.XX -o <out>` with `Range:`/`If-None-Match:` headers passed via `--header`. Parse Status/ETag/Content-Range response. Emit `(200 ok <size> <etag> <lastmod>)` / `(304 not modified)` / `(206 partial <a-b>/<size>)` / `(416 range not satisfiable)`. Exit codes: 0 / 4 / 1 / 4.
- [X] T028 [P] [US4] Implement `cli/mtls-head.sh` — `--name`, `--server`, `--cert`, `--key`, `--cacert`. `curl --cert ... --cacert ... -I`. Print `Status:`, `ETag:`, `Last-Modified:`, `Content-Type:`, `Content-Length:`, `Allow:` lines on stdout.
- [X] T029 [P] [US4] Implement `cli/mtls-del.sh` (DELETE) — `--name`, `--etag`, `--server`, `--cert`, `--key`, `--cacert`. DELETE with optional `If-Match:`. Emit `(deleted <name>)` / `(precondition failed 412)` / `(skipped, conflict 409)`.
- [X] T030 [P] [US4] Implement `cli/mtls-ls.sh` — `--dir` (default: box root), `--server`, `--cert`, `--key`, `--cacert`. PROPFIND `Depth: 1`. Format each child as `<name>\t<size>\t<content-type>\t<etag>\t<lastmod>`. Use `xmllint --xpath` if available, fall back to a 20-line awk-based parser. Exit `4` on non-2xx.
- [X] T031 [P] [US4] Implement `cli/mtls-props.sh` — `--name`, `--server`, `--cert`, `--key`, `--cacert`. PROPFIND `Depth: 0`. Format `<prop> = <value>` lines (`resourcetype`, `getcontentlength`, `getcontenttype`, `getlastmodified`, `getetag`).
- [X] T032 [P] [US4] Implement `cli/mtls-mkcol.sh` (MKCOL) — `--dir`, `--server`, `--cert`, `--key`, `--cacert`. MKCOL. Emit `(201 created dir/<dir>)` / `(409 conflict)`.
- [X] T033 [P] [US4] Implement `cli/mtls-cp.sh` (COPY) — `--source`, `--dest`, `--overwrite`, `--server`, `--cert`, `--key`, `--cacert`. COPY with `Destination: /drop/<cn>/<dest>`; `Overwrite: T` only if `--overwrite` is given. Emit `(201 copied <src> -> <dest>)` or `(412 precondition failed)`.
- [X] T034 [P] [US4] Implement `cli/mtls-mv.sh` (MOVE) — same flags as cp. MOVE with `Destination:` header.
...
- [X] T043 [P] (verified live on openSUSE Tumbleweed-Slowroll: `just test-d` passes (2 D modules); `shellcheck --severity=warning` passes on apache-site.conf.in + handlers/drop-proxy.sh + cli/*.sh; rm-safety grep passes; live e2e curl against httpd 2.4.67 with mod_dav bundled in apache2-prefork confirms: alice PUT /drop/alice.test/notes.txt → 201, alice GET → 200, alice PROPFIND/Depth:1 → 207 (proper multistatus listing `/alice.test/` collection and `/alice.test/notes.txt` resource 19 bytes), bob cross-host /drop/alice.test/... → 403, evil cert (not in trust store) → 401, file actually written to `<data>/drop/alice.test/notes.txt` per `ls`+`cat` — proves mod_dav works end-to-end. Live proof script: `/tmp/dropbox-final.sh`. No separate `apache2-mod_dav` package to zypper install on this distro — `apache2-prefork` ships the .so files.) Run full green-check: `just test-d` (D unit tests — no new D modules added; should still pass unchanged), `just robot` (extended Robot suite incl. drop-box), `bats tests/trust-check.bats tests/apache.bats tests/smoke.bats tests/sync-state.bats tests/migrate-layout.bats`.
- [X] T043a [P] **Post-squash live-verify fix (commit c85cb5d)**. The original squash `2db66f80e9` had two latent config bugs that only surfaced under live-Apache curl:
  - `apache-site.conf.in` loopback VH was missing `DAVLockDB`. mod_dav `PUT`/`MOVE`/`COPY` 500s with `A lock database was not specified with the DAVLockDB directive` (visible in `dav-error.log`). Fix: add `DAVLockDB "{{DATA_DIR}}/apache/dav-lockdb"` inside `<VirtualHost 127.0.0.1:{{DAV_PORT}}>`. Also remove the `AllowDepthInfinity Off` directive — removed in Apache 2.4.x ≥ 2.4.49, no equivalent needed.
  - `handlers/drop-proxy.sh` forwarded to `http://127.0.0.1:${MTLS_DAV_PORT}/drop/${path}` but the loopback VH's `DocumentRoot` is already `<data-dir>/drop`, so PUT landed at `<data-dir>/drop/drop/<cn>/<rest>` (visible as `An error occurred while opening a resource for writing: .../drop/drop/...` in `dav-error.log`). Fix: forward to `http://...${path}` (no extra `/drop/`) and keep the leading slash on `${path}` so the loopback URL stays well-formed.
  Both fixes verified end-to-end with the same `/tmp/dropbox-final.sh` script. Pushed to `okazjonal/main` as `c85cb5d9ee fix(023): add DAVLockDB; drop /drop/ prefix from proxy forward`.
