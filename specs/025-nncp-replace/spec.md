...
**Why `/nncp/receive` matches `nncp-toss`'s defaults**: nncp-toss natively distinguishes — per its `PktTypeArea` branch (`/tmp/nncp-8.13.0/src/toss.go:802-973`) — whether this host is a **full subscriber** (holds `pub + prv` for the area; reads and writes the inner contents), a **relay-only** host (no `prv` for the area, regardless of whether `pub` is present; forwards the inner encrypted blob to subscribers but never decrypts it), or **unconfigured** for the area (no entry at all). The handler does **not** introduce custom relay logic — `handlers/nncp-receive.post.sh` simply feeds bytes in; `nncp-toss` decides every reception outcome. See `## Clarifications` § Session 2026-08-07 / Q1 for the source-grounded rationale.
...
- Q: How should `/nncp/receive` behave when this node's `nncp.hjson` does NOT contain the area private key for an incoming `PktTypeArea`? → A: nncp-toss handles it natively — the tosser relays to subscribers unconditionally (`toss.go:802-848`), logs `rx-area-no-prv` if `prv` is missing (`toss.go:877-880`), and attempts decrypt only if `prv` is present (`toss.go:881-947`). No custom relay logic; the handler stays role-agnostic. The upstream `doc/cfg/areas.texi` `whatever.pvt` example confirms this is a deliberate, supported configuration: "we just relay that area packets to dave and eve, but without ability to see what is inside them."
...
4. **Given** A is configured as a **relay-only** host for a particular area — that is, A's `nncp.hjson` lists this area's `id` (and possibly a `pub`) under the area's entry but **no `prv:`** is present, and A appears in the area's `subs:` list — **When** B sends an `nncp-call --http https://<A>:8443/nncp/receive <A-id> <file>` carrying an inner `PktTypeArea` addressed to that area, **Then** `nncp-toss` (a) does NOT decrypt the inner area layer on A — no payload file appears in `<data-dir>/nncp/spool/area/<area-id>/` for A —, (b) **does** relay the inner encrypted blob to each subscriber under that area's `subs:` list whose `seen/<MsgHash>` file does not yet exist (verified by visiting other peers' `seen/` directories), and (c) emits one `rx-area-no-prv` log line on A confirming the role-and-reason.
...
- **Node is configured as relay-only for an area (no `prv:`, possibly with `pub:`)**: when an inbound `PktTypeArea` reaches `/nncp/receive`, `nncp-toss` (a) relays the inner encrypted blob to each subscriber under the area's `subs:` list whose `seen/` does not yet record the MsgHash, (b) logs `rx-area-no-prv` once per inbound packet, (c) does NOT write a payload file for this area in our SPOOL. Source-grounded at `toss.go:802-973` and in `doc/cfg/areas.texi`'s `whatever.pvt` example: an area config without `prv:` deliberately forwards packets onward without decryption ability. The `/nncp/receive` handler does not need to participate in this branch — `nncp-toss` handles it.
...
- **FR-004 (`/nncp/receive` endpoint)**: A new Apache endpoint registered alongside `/bundle`, `/spool`, `/hello` etc. via `scripts/apache-config.sh` site.conf includes. Method: POST. Body: NNCP-encrypted outer packet bytes. Trust gate: existing per-host mTLS gate from feature 023 (`SSL_CLIENT_CERT` fingerprint match against `<trust_dir>/<cn>.crt`). Handler (`handlers/nncp-receive.post.sh`): writes the body to a temp file, invokes `nncp-toss --pipe --id <our_id> <tmpfile>`, and returns the bytes from `nncp-toss`'s stdout. **`nncp-toss` distinguishes this host's role per-area natively** via its `PktTypeArea` branch (`/tmp/nncp-8.13.0/src/toss.go:802-973`) — full subscriber / relay-only / unconfigured — and the handler is **role-agnostic**: no role-aware dispatch code in `nncp-receive.post.sh`.
...
- **NNCP area role modes**: an area entry in `<data-dir>/nncp.hjson` carries a role implicit from its key-shape, derived by `nncp-toss`'s own logic at `PktTypeArea`: `pub + prv + subs:` = full subscriber (reads & writes inner contents); `pub + subs:` (no `prv`) = relay-with-write (forwards and can submit but cannot decrypt); `id + subs:` only (no `pub`, no `prv`) = pure transit (forwards onward without write or decrypt ability); no entry at all = unconfigured (pkt is logged and dropped). The handler does not parse the role; it forwards bytes to `nncp-toss` and lets `PktTypeArea` branch (`/tmp/nncp-8.13.0/src/toss.go:802-973`) determine behaviour.
...
- **SC-002a (relay-only routing)**: When this node is configured as a relay-only host for a particular area — i.e., the area is listed under `subs:` for this node in `nncp.hjson` but no `prv:` is present — an area-encrypted packet arriving at `/nncp/receive` is **forwarded** by `nncp-toss`'s `PktTypeArea` branch to every other subscriber whose `seen/<MsgHash>` file does not yet exist. No decryption happens on this node; no payload file appears for that area in our SPOOL. Verification: a downstream full-subscriber peer on the LAN reaches the decrypted payload within 5 s of the inbound packet; first-hop's `seen/` directory is populated for each forward target with the BLAKE2b-256(MsgHash) filename as expected; `rx-area-no-prv` is emitted once per inbound packet.
